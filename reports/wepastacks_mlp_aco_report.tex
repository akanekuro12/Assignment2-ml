...
Operational threshold & Fixed at 0.70 & Validation diagnostic & Saved when training is run \\
\bottomrule
\end{tabular}
\end{table}
...
For routing, the metrics are overflow, collected goods, energy, distance, number of sorties, active-warehouse visit rate, runtime, and independently recomputed constraint violations. The primary comparison is MLP--ACO against nearest-first, fullest-first, and ACO-current. A complete routing result requires the entire held-out period and repeated ACO seeds. No operational result is currently retained in the repository.

\section{Results Status}

No trained model, prediction table, routing log, metric table, or generated figure is currently retained. Previous exploratory artifacts were deleted after review because the observed data did not exercise simultaneous multi-warehouse routing and therefore did not provide a meaningful evaluation of ACO route ordering. The report must not reproduce numerical claims from those deleted artifacts.

A new result section may be completed only after a controlled rerun records the exact configuration and demonstrates both parts of the task. Prediction evidence must include class support overall and per warehouse, PR--AUC, ROC--AUC, precision, recall, F1, a confusion matrix at the fixed 0.70 operational threshold, and declared simple baselines. Routing evidence must cover the complete held-out stream with all configured seeds and must report how often zero, one, or multiple warehouses are active simultaneously. If no interval contains at least two feasible active warehouses, the experiment may evaluate dispatch timing but cannot demonstrate ACO route-order optimisation.
...
The research question is currently unanswered because all exploratory result artifacts have been removed. The implementation defines a reproducible experiment, but code execution alone is not evidence that the MLP improves prediction or that MLP filtering improves routing. Any future answer must be based on regenerated artifacts with documented support, baselines, full-period routing, repeated seeds, and constraint checks.
...
The fixed operational threshold is 0.70. A validation-F1 threshold may be saved diagnostically when training is run, but it does not control routing. Because 0.70 was specified rather than selected by an operational validation study, a threshold-sensitivity experiment is necessary. The sigmoid value should be called a risk score until calibration has been demonstrated.
...
\begin{enumerate}
  ...
  \item \textbf{No retained empirical evaluation.} Model and routing artifacts are currently absent, so the report makes no performance claim until a controlled rerun is accepted.
...
No empirical conclusion is currently reported. A future result must first establish adequate positive support for prediction and enough simultaneous active warehouses to exercise route ordering. If those conditions are not met, the project should explicitly limit its claim to implementation and dispatch filtering rather than claim that ACO solves a meaningful multi-stop optimisation problem.
...
\begin{itemize}
  ...
  \item Training recreates \code{model_outputs_single_drone}; no generated model artifact is currently retained.
  \item Accepted prediction evidence must include baseline, support, threshold, confusion-matrix, and per-warehouse tables.
  ...
  \item Routing recreates \code{outputs_single_drone}; no generated routing artifact is currently retained.
  \item Required final evidence: complete held-out routing runs, ten seeds for each stochastic ACO policy, and the distribution of simultaneous active-warehouse counts.
\end{itemize}
